...
// HTML шаблон для входа менеджера
function getManagerLoginEmailTemplate(code: string): string {
  return `
    <!DOCTYPE html>
    <html lang="ru">
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Код входа для менеджера Nur-Kitep</title>
    </head>
    <body style="margin: 0; padding: 0; background-color: #f3f4f6; font-family: Arial, sans-serif;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f3f4f6; padding: 40px 20px;">
        <tr>
            ...
            <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 16px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); overflow: hidden;">
              ...
              <tr>
                <td style="padding: 40px 30px;">
                  ...
                  <p style="color: #4b5563; margin: 0 0 30px 0; font-size: 16px; line-height: 1.6;">
                    Вы запросили вход в панель менеджера. Используйте код ниже для подтверждения:
                  </p>
                  ...
                  <table width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                      <td align="center" style="padding: 30px 0;">
                        <div style="background: linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%); border-radius: 12px; padding: 20px; display: inline-block;">
                          <p style="color: #6b7280; margin: 0 0 10px 0; font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px;">
                            Код подтверждения
                          </p>
              ...
              <tr>
                <td style="background-color: #f9fafb; padding: 30px; text-align: center; border-top: 1px solid #e5e7eb;">
                  <p style="color: #9ca3af; margin: 0 0 10px 0; font-size: 13px;">
                    Если вы не пытались войти в систему, просто проигнорируйте это письмо.
                  </p>
                  <p style="color: #9ca3af; margin: 0; font-size: 13px;">
                    © ${new Date().getFullYear()} Nur-Kitep. Все права защищены.
                  </p>
...
export async function sendManagerLoginEmail(email: string, code: string): Promise<boolean> {
  try {
    ...
    const mailOptions = {
      ...
      text: `Ваш код для входа в панель менеджера: ${code}\n\nКод действителен в течение 10 минут.\n\nЕсли вы не пытались войти в систему, просто проигнорируйте это письмо.`,
